...
# Derived from training distribution.
# Typical athlete ~30% | high-load athlete ~81% | well-rested ~8%
...
if model_ok:
    st.markdown('<div style="text-align:center"><span class="ready-badge">Model loaded and ready</span></div>', unsafe_allow_html=True)
else:
    st.markdown(
        ...
        'injury_model.pkl not found. Place it in the same directory as this script.'
        '</div>',
...
explain_html = (
    ...
    '<p>A probabilistic binary classifier that outputs an injury likelihood score between 0 and 1, '
    'trained on labelled athlete performance data.</p>'
    '</div>'

    '<div class="explain-card" style="--accent:linear-gradient(90deg,#818cf8,#6366f1)">'
    '<div class="icon">&#x2696;&#xFE0F;</div>'
    '<h3>Feature Scaling</h3>'
    '<p>Raw inputs are standardised so every feature contributes proportionally regardless of its unit. '
    'Age, hours, and scores are all normalised before inference.</p>'
    '</div>'

    '<div class="explain-card" style="--accent:linear-gradient(90deg,#e879f9,#d946ef)">'
    '<div class="icon">&#x1F4CA;</div>'
    '<h3>Risk Banding</h3>'
    '<p>The predicted probability maps to three actionable bands: '
    '<strong>Low</strong> (&lt;33%), <strong>Medium</strong> (33-66%), and <strong>High</strong> (&gt;66%).</p>'
    '</div>'
...
st.markdown(
    '<p style="font-size:.85rem;color:#64748b;margin-bottom:.6rem;font-family:\'DM Mono\',monospace;letter-spacing:.06em;">'
    'MODEL INPUT FEATURES</p>',
    unsafe_allow_html=True,
)
feat_items = "".join(
    '<div class="feature-item"><span class="dot"></span>' + f + '</div>'
    for f in EXPECTED_COLUMNS
)
st.markdown('<div class="feature-list">' + feat_items + '</div>', unsafe_allow_html=True)
...
if run_btn and model_ok:
    ...
    try:
        ...
        predicted_class_label = "Injured" if pred == 1 else "Not Injured"
        prob_label = "{:.1%}".format(prob)
        ...
        with r1:
            st.markdown(
                ...
                '<div class="result-value">' + predicted_class_label + '</div>'
        ...
        with r2:
            st.markdown(
                ...
                '<div class="result-value">' + prob_label + '</div>'
        ...
        with r3:
            st.markdown(
                ...
                '<span class="risk-pill ' + bclass + '">' + band + '</span>'
                '</div>'
        ...
        st.progress(min(max(prob, 0.0), 1.0), text="Overall risk level: " + prob_label)

        bd_cards = ""
        for name, val in bd.items():
            val_r = round(val, 1)
            fill  = bar_color(val_r)
            bd_cards += (
                '<div class="breakdown-card">'
                '<div class="b-lbl">' + name + '</div>'
                '<div class="b-val">' + str(val_r) +
                '<span style="font-size:.75rem;font-weight:400;color:#64748b">/100</span></div>'
                '<div class="breakdown-bar">'
                '<div class="breakdown-fill" style="width:' + str(val_r) + '%;background:' + fill + '"></div>'
                '</div>'
                '</div>'
            )
        st.markdown('<div class="breakdown-row">' + bd_cards + '</div>', unsafe_allow_html=True)
